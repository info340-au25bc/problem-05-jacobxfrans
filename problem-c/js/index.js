...
extractOpponent({ "date":"12/31/16", "home":"UW", "opponent":"Alabama", "home_score":7, 
    "opponent_score":24, "passing_yards":150, "rushing_yards":44, "fumbles":1 });
...
const oppos = huskyGames2016.map(extractOpponent);
console.log(oppos);
...
const winners = lostGames.map(extractOpponent);
console.log(winners);
...
lostGames.forEach(game => console.log(game.opponent + " at " + game.home + ", "+ game.opponent_score + " to " + game.home_score));
...
const numFumbles = huskyGames2016.filter(game => game.fumbles > 0);
console.log(numFumbles.length);
...
const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, game => game.fumbles > 0);
...
const lostAndFumbled = huskyGames2016.filter(fumbledAndLostFilter);
console.log(lostAndFumbled);
